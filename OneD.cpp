...
//   
  ...
  double init_value_w 	  = 3;//%-0.6656; //Why these values?
		...
		{	
		  ...
		  // Pacemakers
            ...
            for(mwSize p=0; p<P.size(); p++){
                double dist_from_pace = abs(P[p].x - posx[k]); //distance from pacemaker
            ...
            // Model equations
            double AvgDiff = v[posx[k]+1][posy[k]] + v[posx[k]-1][posy[k]] - 2*v[posx[k]][posy[k]];
