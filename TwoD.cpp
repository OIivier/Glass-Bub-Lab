...
void myspiral(mwSize Nt, double dx, double dt, double D, double wH, double wLP, 
		    ...
		    double *outVid, double *outVidW, double *outPace, double *VCoeff, double *VCoeffMod, int isRotor, mwSize SizeX, mwSize SizeY)  
{
	  ...
	  if(n<MapModTime){
		...
		{	
		  ...
		  // Pacemakers
            ...
            for(mwSize p=0; p<P.size(); p++){
                ...
                if(isRotor==0){ //If it's not a rotor, it's a pacemaker
                ...
                if(n==0 && isRotor==1){ //If it *is* a rotor...
            ...
            // Model equations end here
		
            for(mwSize p=0; p<P.size(); p++){
                if(posx[k]==P[p].x && posy[k]==P[p].y && n>pon)
                {
            ...
            outVidW[posy[k] + posx[k]*Ny + Nx*Ny*n/SKIP_VDATA] = w[posx[k]][posy[k]];
            }
		...
		{	
		  for(int k=0; k < nPosXYMod; ++k){
            ...
            for(mwSize p=0; p<P.size(); p++){
                if(posxMod[k]==P[p].x && posyMod[k]==P[p].y && n>pon)
                {
            ...
            if(n % SKIP_VDATA==0){
                ...
                outVidW[posyMod[k] + posxMod[k]*Ny + Nx*Ny*n/SKIP_VDATA] = w[posxMod[k]][posyMod[k]]; 
            }
                  ...
                  int nrhs, const mxArray *prhs[])
{
    ...
    nPosXYMod 	= mxGetScalar(prhs[14]);
    PosXMod	= mxGetPr(prhs[15]); 
    PosYMod	= mxGetPr(prhs[16]); 
    MapModTime = mxGetScalar(prhs[17]);
    VCoeff	= mxGetPr(prhs[18]);
    VCoeffMod = mxGetPr(prhs[19]);
    isRotor = mxGetScalar(prhs[20]);
    SizeX = mxGetScalar(prhs[21]);
    SizeY = mxGetScalar(prhs[22]);
    ...
    plhs[1] = mxCreateNumericArray(3, dimVid, mxDOUBLE_CLASS, mxREAL);	// videoW
    ...
    myspiral(Nt, dx, dt, D, wH, wLP, pon, poff, pdx, pdy, TotalBeats, 
		    ...
		    outVid, outVidW, outPace, VCoeff, VCoeffMod, isRotor, SizeXmw, SizeYmw); 
}
