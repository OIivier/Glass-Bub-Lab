...
Fibr = 0.00; %Chance to have "fibrosis" (block), in [0,1]
Vert = 0; %Vert elongation of fibrosis, in [0,infty[
MaxRadius = 0;
        ...
        if iMapX-Radius>0 && Radius+iMapX<MapDimensions && iMapY-Radius>0 && Radius+iMapY<MapDimensions
            for iFibrX=iMapX-Radius:iMapX+Radius
                for iFibrY=iMapY-Radius:iMapY+Radius
                    if ceil(sqrt((iFibrX-iMapX)^2+(Vert+1)*(iFibrY-iMapY)^2))<=Radius
                        Map(iFibrX,iFibrY) = 0;
                    end
                end
            end
...
% ======================================================================= %
tic
[m, Toff] = px_ch(Nt, dx, dt, D, wH, wLP, pon, poff, pdx, pdy, nBeats, size(PosX,1), PosX, PosY, NoiseSignal, NoiseRecord);
toc
% ======================================================================= %
