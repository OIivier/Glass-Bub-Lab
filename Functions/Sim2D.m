...
%Here, you should have a variable called Map, which is an array of values
%between 0 and 1
...
dt = 0.025;      % [tau]
dx = 0.025;      % [cm]
D = 0.0003;      % [cm^2/tau] diffusion coeff
D_cm = D/tau;    % [cm^2/s]
...
tic
[m, mW, Toff] = TwoD(Nt, dx, dt, D, wH, wLP, pon, poff, pdx, pdy, nBeats, size(PosX,1), PosX, PosY, size(PosXMod,1), PosXMod, PosYMod, MapModTime, VCoeff, VCoeffMod, isRotor, SizeX, SizeY);
toc
...
if showMovie==1
    ...
    DataSkipFrames = m(4:end,4:end,1:40:end);

    DataSkipFramesW = mW(:,:,1:40:end);
    nFrames = size(DataSkipFrames,3);
    F(nFrames) = struct('cdata',[],'colormap',[]);

% I use the following couple of lines when I want to do the phase plane for one point, Data1
% being the time series of a single point
...
%     w2 = v+Beta;%./(Gamma.*(wH-wL./(1+exp(-4*v))+wL));%dw/dt =0
    ...
    for j = 1:round(nFrames/1) %replace "1" by a bigger number to get shorter movie
    %     plot(DataSmall(2:Length-2,4,j)');
        s = surf(DataSkipFrames(:,:,j)');
        s.EdgeColor = 'none';
        caxis([-1.95 1.7]); %scale of the color map
        view([0 90]); %view from above so that it's 2D (change angle to get different view)
