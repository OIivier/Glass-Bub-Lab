function [M,N,NumFrames,NumCols,NumRows,FrameInt,AcquisitionRatio,ibis,ActMap,AmpThresh] = daReadMod(daFile)
...
MaxData=max(DataVec);MinData=min(DataVec);MeanData=mean(DataVec);
%DataSTD=std(DataVec)
...
for y = 1:NumRows
    for x = 1:NumCols
        filt=N(x,y,:);
        thresh=(max(filt)+min(filt));
        if (max(N(x,y,:))-min(N(x,y,:)))>AmpThresh
            for t=1:NumFrames-2
                if N(x,y,t)<thresh
                    if N(x,y,t+1)>=thresh
                        ibis(x,y,t)=1;
