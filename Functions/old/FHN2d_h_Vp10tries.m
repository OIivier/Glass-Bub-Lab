function Vp=FHN2d_h_Vp10tries(n,D,time);
...
for t=ltI:time/dt; %2nd for loop is identical to 1st except without injected current. also here we calcualte Vp
    ...
    for j=1:10;
        if (v(20*j,50,2)>=0.3) && (v(20*j,50,1)<0.3)  %we catch v on the upstroke and make sure it's not a sink.
         t1(j,t)=t; %this sets up so that we keep only first time that this happens.
        else
            t1(j,t)=0;
        end 
        if (v(20*j,200,2)>=0.3) && (v(20*j,200,1)<0.3) 
          t2(j,t)=t;
        else
            t2(j,t)=0;
        end 
