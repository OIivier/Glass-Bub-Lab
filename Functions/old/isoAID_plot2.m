function [a,b,c,g]=isoAID_plot2(N,thresh,thresh2,start,fin)
...
for i=1:m
    for j=1:m
        ...
        l=1;
        for t=1:(nsteps-1)  
           if (N(i,j,t+start-1)<thresh && N(i,j,t+start)>thresh)
                a(i,j,k)=((t+start-2)/sr)+(thresh-N(i,j,(t+start-1)))/(sr*(N(i,j,t+start)-N(i,j,t+start-1)));
                
                if k==1 %this checks if your activation time is a abnormally late, and sets it to zero (undetected) if it is. 
                    if (a(i,j,k))>(starttime+(endtime-starttime)/3)
                        a(i,j,k)=0;
                    end
                end
                if k==2
                    if (a(i,j,k))>(starttime+(endtime-starttime)*2/3)
                        a(i,j,k)=0;
                    end
                end
