function [cycleLength apdA apdB apdC DataPoint] = Sim1D(wL, showMovie, nBeats, RealLength, dx, RealDuration, dt, APDPerc, Pacemaker1CM, Pacemaker2CM, Ip1, Ip2, wLP1, wLP2, Obstacle, ObstacleWidth, ObstacleTime);
...
if ~exist('Pacemaker2CM','var')
    Pacemaker2CM = RealLength+1;
end

Pacemaker1 = round(Pacemaker1CM/dx);
Pacemaker2 = round(Pacemaker2CM/dx);
...
if exist('NumberOfBeats','var')
    toAppend = ['nBeats',num2str(nBeats),'_'];
    file.name = strcat(file.name,toAppend);
end
...
if showMovie==1
    ...
    Data1 = squeeze(Data(round(size(Data,1)/20),:));
    DataW1 = squeeze(DataW(round(size(DataW,1)/20),:));

    Data2 = squeeze(Data(round(19*size(Data,1)/20),:));
    DataW2 = squeeze(DataW(round(19*size(DataW,1)/20),:));
...
%% RECOVERY PERIOD AND APD

% apdA is width of peaks (like FWHM) 
% apdB is width from last minimum
% apdC is width from threshold
...
%%
DataPointDerivative = DataPoint;
for i=1:size(DataPoint,1)-1
    DataPointDerivative(i) = DataPoint(i+1)-DataPoint(i);
end
DataPointSecondDerivative = DataPointDerivative;
for i=1:size(DataPointDerivative,1)-1
    DataPointSecondDerivative(i)=DataPointDerivative(i+1)-DataPointDerivative(i);
end
% plot(DataPoint,'Color','b');
% hold on
% plot(DataPointDerivative,'Color','r');
% plot(DataPointSecondDerivative,'Color','k');

% 
% 
% for i=1:size(DataPoint,1)-1
%     if abs(DataPointSecondDerivative(i))<0.0001 && DataPointSecondDerivative(i+1)>0.0001
%         text(i,DataPoint(i),'x')
%     end
% end
...
for i=2:size(peaks,1)-1
    if peaks(i)>threshold
        ...
        thresholdValue = 0;
        for k=locs(i-1):locs(i)
            if abs(DataPointSecondDerivative(k))<0.0001 && DataPointSecondDerivative(k+1)>0.0001
            	thresholdValue=DataPoint(k);
            end
        end
        ...
        while locs(i)-j>=1 && DataPoint(locs(i)-j)~=minimumBefore;
            if frameBeforeA==0 && abs(DataPoint(locs(i)-j)-peaks(i))<abs(peaks(i)-minimumBefore)*APDPerc/100;
                frameBeforeA = j;
            end
            if frameBeforeC==0 && DataPoint(locs(i)-j)<thresholdValue
                frameBeforeC = j;
            end
            frameBeforeB = j;
            j = j+1;
        ...
        apdASum = apdASum+frameAfter+frameBeforeA;
        apdBSum = apdBSum+frameAfter+frameBeforeB;
        apdCSum = apdCSum+frameAfter+frameBeforeC;
        lastPeak=locs(i,1);
        nPeaks = nPeaks+1;
    end
end
...
apdA = (dt^2)*apdASum/nPeaks;
apdB = (dt^2)*apdBSum/nPeaks;
apdC = (dt^2)*apdCSum/nPeaks;
nPeaks = nPeaks-SkipNFirstPeaks;
cycleLength = (lastPeak-firstPeak)/nPeaks;
